...
> **说明**：所有预订、订座、下单动作为 Mock API 演示，使用本地模拟数据（北京餐厅和活动），不涉及真实美团接口。
...
打开页面后，直接输入以下任一语句即可看到完整闭环：

| 场景 | 推荐输入 |
|------|---------|
| 家庭亲子 | `今天下午想带 5 岁孩子和老婆出去玩几个小时，别离家太远，老婆最近在减肥` |
| 朋友聚会 | `今天下午 4 个朋友出去玩，2 男 2 女，想有互动也要吃饭，别太远` |
| 餐厅无位恢复 | `今天下午带老婆孩子出去玩，餐厅满了也要自动换一个` |
| 情侣约会 | `周末下午和女朋友出去玩，想浪漫一点，预算 500 左右` |

输入后等待方案生成 → 点击确认 → 观察工具调用时间线和 Mock 预订结果。
